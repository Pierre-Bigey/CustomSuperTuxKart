...
################## Part 4: compute the 3D calcposition ##################
...
def visualize(
  ...
  """
    ...
    for detection in detection_result.detections:
        ...
        # Indices for the left and right eye keypoints
        ...
        cv2.circle(annotated_image, left_eye_px, thickness, color, radius)
        cv2.circle(annotated_image, right_eye_px, thickness, color, radius)

        # Draw the center of the eyes with a different color
        center_eye = (int((left_eye_px[0] + right_eye_px[0]) / 2), int((left_eye_px[1] + right_eye_px[1]) / 2))
        cv2.circle(annotated_image, center_eye, thickness, (0, 0, 255), radius)
...
def compute3DPos(ibe_x, ibe_y, rec_ipd):
    ...
    # We consider that the head faces the camera ( the head is orthogonal to the camera main axis)
    z = fl * user_ipd / rec_ipd


    # compute the x and y coordinate in a Yup reference frame
    center_x = screen_width_px / 2
    center_y = screen_heigth_px / 2
    x = (ibe_x - center_x) * user_ipd / rec_ipd
    y = (ibe_y - center_y) * user_ipd / rec_ipd
...
def runtracking():
    ...
    while True:
        ...
        if res.tracking_results != None:
            ...
            max_area = 0
            face = None
            for detection in res.tracking_results.detections:
            ...
            if face is None:
                continue
            res.tracking_results.detections = [face]

            # Get the position of the two eyes in pixels
            left_eye = face.keypoints[1]
            right_eye = face.keypoints[0]
            left_eye_x, left_eye_y = _normalized_to_pixel_coordinates(left_eye.x, left_eye.y, screen_width_px, screen_heigth_px)
            right_eye_x, right_eye_y = _normalized_to_pixel_coordinates(right_eye.x, right_eye.y, screen_width_px, screen_heigth_px)

            # If we have a position for the two eyes
            # compute the position between the two eyes (in pixels)
            ibe_x = (left_eye_x + right_eye_x) / 2
            ibe_y = (left_eye_y + right_eye_y) / 2

            # compute the interpupillary distance (in pixels)
            rec_ipd = math.sqrt((left_eye_x - right_eye_x)**2 + (left_eye_y - right_eye_y)**2)
